...
Gitea is a core component. The Thinkube installer installs it by running
`00_install.yaml`, which runs `10_deploy.yaml`, `15_configure.yaml`,
`16_configure_ssh_keys.yaml` and `17_configure_discovery.yaml` in that order.
It is not installed on its own. `13_configure_code_server.yaml` and
`14_configure_sso_user.yaml` are not part of `00_install.yaml`.
...
- **Storage**: 10Gi PVC for repositories, storage class `k8s-hostpath`

## Purpose in Thinkube

Gitea solves a critical problem in multi-domain deployments:
1. **Templates** in GitHub use variables like `{{ domain_name }}`
2. **Deployments** need actual values like `registry.<domain_name>`
...
This creates the initial organizations and the deployment repository structure.

### Step 3: SSH Keys and Service Discovery
```bash
./scripts/run_ansible.sh ansible/40_thinkube/core/gitea/16_configure_ssh_keys.yaml
./scripts/run_ansible.sh ansible/40_thinkube/core/gitea/17_configure_discovery.yaml
```

`16_configure_ssh_keys.yaml` creates one SSH key for all Gitea repositories
and puts it in the shared-code directory for code-server.
...
# Use in API calls
curl -H "Authorization: token YOUR_TOKEN" https://git.<domain_name>/api/v1/user
```
